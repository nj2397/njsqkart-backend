...
const getUser = catchAsync(async (req, res) => { 
    ...
    if(userDetails.email !== req.user.email){
      throw new ApiError(httpStatus.FORBIDDEN, "User not authenticated to access this resource");
    }

    if(Object.keys(req.query).length > 0){
      // res.send(req.params.userId);
      const userDetails = await userService.getUserAddressById(req.params.userId);
      if(!userDetails){
